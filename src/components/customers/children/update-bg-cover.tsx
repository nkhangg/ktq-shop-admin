...
export interface ICUpdateBgCoverProps {
    customerId: ICustomer['id'];
}

export default function CUpdateBgCover({ customerId }: ICUpdateBgCoverProps) {
    ...
    const updateMutation = useMutation({
        mutationFn: (data: string) => customersApi.updateBgCover(customerId, data),
        mutationKey: ['customers/update-bg-cover/[POST]'],
        onSuccess(response) {
    ...
    const deleteMediaMutation = useMutation({
        ...
        mutationKey: ['customers/delete-bg-cover/[DELETE]'],
        onSuccess(response) {
            Api.handle_response(response);
    ...
    const handleUpdate = (data: string) => {
        dispatch(
            addComfirm({
                title: 'Are you want to update bg cover for customer',
    ...
    const handleDeleteMedia = (e: MouseEvent<HTMLButtonElement>) => {
        ...
        dispatch(
            addComfirm({
                title: `Are you want to delete background cover`,
                callback: () => deleteMediaMutation.mutate('bg_cover'),
            }),
        );
    };
                ...
                <Group justify="center" gap="xl" p={10} style={{ pointerEvents: 'none' }} className="border-2 border-gray-500 rounded-lg">
                    ...
                    <Dropzone.Idle>
                        <Box style={{ pointerEvents: 'auto' }} className="flex items-center justify-center flex-col">
                            {data?.customer.bg_cover ? (
                                <Box className="group relative">
                                    <Image radius="md" h={280} src={data?.customer.bg_cover} />
                                ...
                                <>
                                    ...
                                    <Text className="text-center">Set Background Cover</Text>
                                </>
                            )}
                        </Box>
                    </Dropzone.Idle>
